  ...
  return (
    <div className="flex h-full flex-col bg-[#fbfcfd] dark:bg-slate-950">
      <div className="border-b border-slate-200 bg-[#f6f7f9] px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-[13px] font-semibold text-slate-700 dark:text-slate-200">AI 策略助手</div>
              <div className="text-[11px] text-slate-500 dark:text-slate-400">{MODE_CONFIG[mode].description}</div>
            </div>
          </div>
          <Badge variant={loading ? "default" : "secondary"} className={cn("text-[11px]", loading && "animate-pulse")}>
            {loading ? "处理中..." : "就绪"}
          </Badge>
        </div>
      </div>
      ...
      >
        <TabsList
          variant="line"
          className="h-auto w-full grid grid-cols-3 rounded-none border-b border-slate-200 bg-white p-0 dark:border-slate-800 dark:bg-slate-950"
        >
          {(Object.keys(MODE_CONFIG) as CopilotMode[]).map((m) => (
            <TabsTrigger
              key={m}
              value={m}
              className="flex-1 rounded-none border-b-2 border-transparent py-2.5 text-[12px] data-active:border-slate-900 data-active:bg-transparent dark:data-active:border-slate-100"
            >
