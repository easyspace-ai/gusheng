...
import (
	...
	tencentkline "github.com/easyspace-ai/tusharedb-go/pkg/tencentkline"
	"gorm.io/gorm"
	"gorm.io/gorm/clause"

	"obsidianfs/internal/core/domain/stock"
	"obsidianfs/internal/core/ports"
	"obsidianfs/internal/infrastructure/cache"
	"obsidianfs/internal/infrastructure/clients"
	"obsidianfs/internal/infrastructure/clients/eastmoney"
	"obsidianfs/internal/infrastructure/clients/sina"
)
...
func (r *StockRepositoryImpl) Search(keyword string) ([]stock.StockInfo, error) {
	cacheKey := cache.GenerateKey("stock", "search", keyword)
	var results []stock.StockInfo
...
// FollowStock 关注股票
func (r *StockRepositoryImpl) FollowStock(userId, code, name string) error {
	var count int64
	r.db.Model(&stock.FollowedStock{}).Where("user_id = ? AND stock_code = ?", userId, code).Count(&count)
	if count > 0 {
		return nil
	}
