...
export async function listBacktestStrategies(): Promise<BacktestStrategySummary[]> {
  return request<BacktestStrategySummary[]>('/api/backtest/api/v1/strategies?limit=200')
}
...
export async function listBacktestJobs(params?: {
  ...
  return request<BacktestJob[]>(`/api/backtest/api/v1/backtests${query ? `?${query}` : ''}`)
}

export async function runBacktest(requestBody: BacktestRunRequest): Promise<BacktestJob> {
  return request<BacktestJob>('/api/backtest/api/v1/backtests/run', {
    method: 'POST',
...
export async function getBacktestDetail(jobId: string): Promise<BacktestDetailResponse> {
  return request<BacktestDetailResponse>(`/api/backtest/api/v1/backtests/${jobId}/detail`)
}
