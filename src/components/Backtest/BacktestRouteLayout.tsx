...
function EquityChart({
  ...
  if (!data.length) {
    return (
      <div className="flex h-72 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-white/70 text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-400">
        暂无权益曲线数据
      </div>
    )
  }
...
export function BacktestRouteLayout() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      ...
      try {
        ...
        setSymbolSearchResults(response.results.length ? response.results : FALLBACK_SYMBOLS)
      } catch {
  ...
  const summaryCards = useMemo(() => {
    ...
    return [
      ...
      { label: '交易数', value: String(result?.total_trades ?? metricNumber(metrics, 'total_trades') ?? trades.length ?? 0), tone: 'text-slate-700' },
  ...
  const leftPanel = (
    <div className="flex h-full flex-col bg-[#fbfcfd] dark:bg-slate-950">
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-800">
        <div>
          <div className="text-[11px] uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">Backtest Desk</div>
          <h2 className="mt-1 text-[13px] font-semibold text-slate-900 dark:text-slate-100">标的与参数</h2>
        </div>
        ...
        <div className="space-y-5 p-4">
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">选股</div>
              <span className="text-[11px] text-slate-400">{symbolLoading ? '搜索中...' : '按代码或名称检索'}</span>
            </div>
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  ...
                  className="h-10 rounded-xl border-slate-200 bg-white pl-9 text-sm shadow-none transition-shadow focus-visible:ring-0 dark:border-slate-800 dark:bg-slate-950"
                  placeholder="输入股票代码或名称"
                />
              </div>
              <button
                onClick={() => applySymbol(symbolInput, symbolSearchResults[0]?.name)}
                className="inline-flex h-10 items-center gap-1 rounded-xl bg-slate-900 px-3.5 text-sm font-medium text-white transition-colors hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white"
              >
                确认
                <ArrowRight size={14} />
              </button>
            ...
            <div className="flex flex-wrap gap-2">
              {FALLBACK_SYMBOLS.map((item) => {
                const active = selectedSymbol === item.symbol
                return (
                  <button
                    key={item.symbol}
                    onClick={() => applySymbol(item.symbol, item.name)}
                    className={cn(
                      'rounded-full border px-3 py-1.5 text-xs transition-colors',
                      active
                        ? 'border-slate-300 bg-slate-100 text-slate-800 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100'
                        : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400 dark:hover:border-slate-700',
                    )}
                  >
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 text-sm dark:border-slate-800 dark:bg-slate-900/50">
              <div className="text-[11px] uppercase tracking-[0.18em] text-slate-400">当前标的</div>
              <div className="mt-1 text-base font-semibold text-slate-900 dark:text-slate-100">
                {selectedSymbol} · {selectedSymbolName || '等待匹配名称'}
              </div>
              <div className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
                左侧栏负责输入标的、策略和参数，中间区域专注展示回测结果与历史明细。
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2">
              {symbolSearchResults.slice(0, 6).map((item) => (
                <button
                  key={item.symbol}
                  onClick={() => applySymbol(item.symbol, item.name)}
                  className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-left text-xs text-slate-600 transition-colors hover:border-slate-300 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300 dark:hover:border-slate-700 dark:hover:bg-slate-900"
                >
                  <div className="font-semibold">{item.symbol}</div>
                  <div className="mt-0.5 truncate text-[11px] opacity-80">{item.name}</div>
                </button>
              ))}
            </div>
          </section>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">策略</div>
              <span className="text-[11px] text-slate-400">{strategiesLoading ? '加载中...' : `${strategies.length} 个策略`}</span>
            </div>
            ...
            <div className="space-y-2">
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      setStrategyParameters(buildDefaultParameters(strategy.parameters_schema))
                    }}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{strategy.name}</div>
                      <span className={cn('rounded-full border px-2 py-0.5 text-[10px] uppercase tracking-[0.18em]', statusTone(strategy.status))}>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2 text-[11px] text-slate-400">
                      {strategy.version ? <span>v{strategy.version}</span> : null}
                      {strategy.is_template ? <span>模板</span> : null}
                      {strategy.tags?.slice(0, 2).map((tag) => (
                        <span key={tag} className="rounded-full bg-slate-100 px-2 py-0.5 dark:bg-slate-900">
                          {tag}
                        </span>
                      ))}
                    </div>
              ...
              {!strategiesLoading && !strategies.length ? (
            ...
            {selectedStrategy ? (
              <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-800 dark:bg-slate-900/50">
                <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{selectStrategyLabel(selectedStrategy)}</div>
          ...
          <section className="space-y-3">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">参数</div>
            ...
            {parameterFields.length ? (
                    ...
                    <label key={field.key} className="block">
                      ...
                      {field.enum?.length ? (
                        <select
                          ...
                          className="h-10 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-none outline-none transition focus:border-slate-300 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100"
                        >
                        ...
                        <button
                          ...
                          className={cn(
                            'flex h-10 w-full items-center justify-between rounded-xl border px-3 text-sm transition-colors',
                            enabled
                        ...
                        <Input
                          ...
                          className="h-10 rounded-xl border-slate-200 bg-white text-sm shadow-none focus-visible:ring-0 dark:border-slate-800 dark:bg-slate-950"
                        />
                      )}
                      {field.description ? (
                        <div className="mt-1 text-[11px] leading-5 text-slate-400">{field.description}</div>
                      ) : null}
                    </label>
                  )
                })}
              ...
              <div className="space-y-2">
                <Textarea
                  ...
                  className="min-h-40 rounded-xl border-slate-200 bg-white font-mono text-xs shadow-none focus-visible:ring-0 dark:border-slate-800 dark:bg-slate-950"
                />
                <div className="text-[11px] leading-5 text-slate-400">
                  该策略暂无结构化参数 schema，先用 JSON 直接编辑，后续可接 AI 自动生成的参数面板。
                </div>
              </div>
            )}
          </section>
          ...
          <section className="space-y-3">
            <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">运行窗口</div>
            <div className="grid grid-cols-2 gap-2">
              <label className="space-y-1">
                <span className="text-[11px] text-slate-400">开始日期</span>
                <Input type="date" value={runStartDate} onChange={(event) => setRunStartDate(event.target.value)} className="h-10 rounded-xl border-slate-200 bg-white text-sm dark:border-slate-800 dark:bg-slate-950" />
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-slate-400">结束日期</span>
                <Input type="date" value={runEndDate} onChange={(event) => setRunEndDate(event.target.value)} className="h-10 rounded-xl border-slate-200 bg-white text-sm dark:border-slate-800 dark:bg-slate-950" />
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-slate-400">初始资金</span>
                <Input type="number" value={initialCapital} onChange={(event) => setInitialCapital(event.target.value)} className="h-10 rounded-xl border-slate-200 bg-white text-sm dark:border-slate-800 dark:bg-slate-950" />
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-slate-400">手续费</span>
                <Input type="number" step="0.0001" value={commissionRate} onChange={(event) => setCommissionRate(event.target.value)} className="h-10 rounded-xl border-slate-200 bg-white text-sm dark:border-slate-800 dark:bg-slate-950" />
              </label>
              <label className="space-y-1">
                <span className="text-[11px] text-slate-400">滑点</span>
                <Input type="number" step="0.0001" value={slippage} onChange={(event) => setSlippage(event.target.value)} className="h-10 rounded-xl border-slate-200 bg-white text-sm dark:border-slate-800 dark:bg-slate-950" />
              </label>
              <label className="space-y-1">
            ...
            <button
              ...
              className={cn(
                'flex h-11 w-full items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold transition-colors',
                !selectedStrategy || runStatus === 'running'
                  ? 'cursor-not-allowed bg-slate-200 text-slate-500 dark:bg-slate-800 dark:text-slate-400'
                  : 'bg-slate-900 text-white hover:bg-slate-800 dark:bg-slate-100 dark:text-slate-900 dark:hover:bg-white',
              )}
            >
              {runStatus === 'running' ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />}
              {runStatus === 'running' ? '回测中...' : '开始回测'}
            </button>
            <div className="text-[11px] leading-5 text-slate-400">{runMessage}</div>
          </section>
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">最近回测</div>
              <button onClick={refreshHistory} className="text-[11px] text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200">
                刷新
              </button>
            </div>
            {historyError ? (
            ...
            <div className="space-y-2">
              ...
              {strategyHistory.map((job) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">{job.symbol}</div>
    ...
    <div className="flex h-full min-w-0 flex-col bg-white dark:bg-slate-950">
      <div className="border-b border-slate-200 px-5 py-4 dark:border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-slate-400 dark:text-slate-500">Backtest Result</div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className={cn('rounded-full border px-3 py-1 text-xs font-medium', statusTone(runStatus === 'idle' ? activeDetail?.job.status : runStatus))}>
      ...
      <ScrollArea className="flex-1">
        <div className="space-y-5 bg-[#f3f5f7] p-5 dark:bg-slate-950">
          {detailLoading ? (
            <div className="flex h-56 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-950/40 dark:text-slate-400">
          ...
          {!detailLoading && !activeDetail ? (
            <motion.div
              ...
              className="rounded-xl border border-dashed border-slate-200 bg-white p-8 shadow-[0_18px_60px_rgba(15,23,42,0.04)] dark:border-slate-800 dark:bg-slate-950/40"
            >
              <div className="flex items-start justify-between gap-6">
                <div className="max-w-2xl">
                  <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.2em] text-slate-500 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-400">
                    <Workflow size={12} />
                    回测工作台
                  </div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-800 dark:bg-slate-950">日频回测</span>
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-800 dark:bg-slate-950">A 股标的</span>
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-800 dark:bg-slate-950">策略参数</span>
                    <span className="rounded-full border border-slate-200 bg-white px-3 py-1 dark:border-slate-800 dark:bg-slate-950">历史结果</span>
                  </div>
                </div>
                <div className="hidden rounded-xl border border-slate-200 bg-slate-50 p-4 text-slate-500 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400 lg:block">
          ...
          {activeDetail ? (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div className="max-w-2xl">
                    <div className="text-xs uppercase tracking-[0.18em] text-slate-400">结果摘要</div>
                  ...
                  <div className="flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-500 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400">
                    <Activity size={14} />
                    {runMessage}
                  </div>
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                  <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                    <div className="text-[11px] uppercase tracking-[0.16em] text-slate-400">总收益</div>
                    <div className={cn('mt-1 text-xl font-semibold', summaryCards[0]?.tone)}>{summaryCards[0]?.value}</div>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                    <div className="text-[11px] uppercase tracking-[0.16em] text-slate-400">年化 / 回撤</div>
                    <div className="mt-1 text-xl font-semibold text-slate-900 dark:text-slate-100">
                      {summaryCards[1]?.value} / {summaryCards[2]?.value}
                    </div>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                    <div className="text-[11px] uppercase tracking-[0.16em] text-slate-400">交易数</div>
                    <div className={cn('mt-1 text-xl font-semibold', summaryCards[4]?.tone)}>{summaryCards[4]?.value}</div>
                  </div>
                  <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                    <div className="text-[11px] uppercase tracking-[0.16em] text-slate-400">胜率 / 夏普</div>
                    <div className="mt-1 text-xl font-semibold text-slate-900 dark:text-slate-100">
              ...
              {activeDetail.result ? (
                <>
                  <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
                    {summaryCards.map((card) => (
                      <div key={card.label} className="rounded-xl border border-slate-200 bg-white p-4 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                  ...
                  <section className="grid gap-4 xl:grid-cols-[1.35fr_0.95fr]">
                    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                      <div className="flex items-center justify-between gap-4">
                        <div>
                          <div className="text-xs uppercase tracking-[0.18em] text-slate-400">权益曲线</div>
                          <div className="mt-1 text-sm font-semibold text-slate-900 dark:text-slate-100">账户价值与回撤</div>
                        </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                        <div className="text-xs uppercase tracking-[0.18em] text-slate-400">回测概况</div>
                        <div className="mt-4 space-y-3 text-sm text-slate-600 dark:text-slate-400">
                          <div className="flex items-center justify-between gap-4">
                            <span>策略</span>
                            <span className="font-medium text-slate-900 dark:text-slate-100">{activeDetail.result.strategy_name}</span>
                          </div>
                          <div className="flex items-center justify-between gap-4">
                            <span>标的</span>
                            <span className="font-medium text-slate-900 dark:text-slate-100">{activeDetail.result.symbol}</span>
                          </div>
                          <div className="flex items-center justify-between gap-4">
                            <span>区间</span>
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <span>初始资金</span>
                            <span className="font-medium text-slate-900 dark:text-slate-100">{formatNumber(activeDetail.result.initial_capital)}</span>
                          </div>
                          <div className="flex items-center justify-between gap-4">
                            <span>最终资金</span>
                            <span className="font-medium text-slate-900 dark:text-slate-100">{formatNumber(activeDetail.result.final_capital)}</span>
                          </div>
                        </div>
                      </div>

                      <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                        <div className="text-xs uppercase tracking-[0.18em] text-slate-400">运行信息</div>
                        <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                            <div className="text-[11px] text-slate-400">回测任务</div>
                            <div className="mt-1 font-medium text-slate-900 dark:text-slate-100">{activeDetail.job.job_id.slice(0, 10)}</div>
                          </div>
                          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                            <div className="text-[11px] text-slate-400">任务状态</div>
                            <div className="mt-1 font-medium text-slate-900 dark:text-slate-100">{activeDetail.job.status}</div>
                          </div>
                          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                            <div className="text-[11px] text-slate-400">提交时间</div>
                          ...
                          <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-900/60">
                            ...
                            <div className="mt-1 font-medium text-slate-900 dark:text-slate-100">{formatPercent(activeDetail.job.progress)}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-[0_16px_60px_rgba(15,23,42,0.05)] dark:border-slate-800 dark:bg-slate-950/70">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        <div className="text-xs uppercase tracking-[0.18em] text-slate-400">交易明细</div>
                        <div className="mt-1 text-sm font-semibold text-slate-900 dark:text-slate-100">买卖记录与手续费</div>
                      </div>
                      <div className="flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1 text-xs text-slate-500 dark:border-slate-800 dark:text-slate-400">
                        <SquareStack size={14} />
                        {trades.length} 笔
                        ...
                        <TableBody>
                          {trades.length ? trades.map((trade) => (
                            <TableRow key={trade.trade_id}>
                              <TableCell className="font-mono text-xs text-slate-500">{formatDate(trade.trade_time)}</TableCell>
                              <TableCell>
                ...
                <section className="rounded-xl border border-dashed border-slate-200 bg-white p-6 shadow-[0_16px_60px_rgba(15,23,42,0.04)] dark:border-slate-800 dark:bg-slate-950/70">
                  <div className="flex items-start justify-between gap-6">
                    <div className="max-w-2xl">
                      <div className="text-xs uppercase tracking-[0.18em] text-slate-400">结果同步中</div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-500 dark:text-slate-400">
                        <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 dark:border-slate-800 dark:bg-slate-900/60">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-slate-500 dark:border-slate-800 dark:bg-slate-900/50 dark:text-slate-400">
  ...
  const rightPanel = (
    <div className="flex h-full flex-col bg-[#fbfcfd] dark:bg-slate-950">
      <div className="border-b border-slate-200 px-4 py-3 dark:border-slate-800">
        <div className="text-[11px] uppercase tracking-[0.22em] text-slate-400">Reserve Panel</div>
        <h3 className="mt-1 text-[13px] font-semibold text-slate-900 dark:text-slate-100">右侧预留区</h3>
      </div>
      <div className="flex flex-1 items-center justify-center p-5">
        <div className="max-w-xs rounded-xl border border-dashed border-slate-200 bg-slate-50/80 p-5 text-center dark:border-slate-800 dark:bg-slate-900/40">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl border border-slate-200 bg-white text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
            <Target size={20} />
          </div>
          <div className="mt-4 text-sm font-semibold text-slate-900 dark:text-slate-100">暂时留白</div>
