...
export function StrategyRouteLayout() {
  ...
  const editorTheme = isDarkMode ? "custom-dark" : "light";
  ...
  const leftPanel = (
    <div className="flex h-full flex-col bg-slate-100/90 backdrop-blur-xl dark:bg-slate-900/90">
      <div className="flex items-center justify-between border-b border-slate-200/80 px-4 py-3 dark:border-slate-800/80">
        <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
          Strategies
        </h2>
        <div className="flex items-center gap-1">
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200/80 bg-white/90 text-slate-600 transition-colors hover:bg-white dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-800/80"
            title="新建策略"
          >
            <Plus size={16} />
          </button>
      ...
      <div className="border-b border-slate-200/80 px-4 py-3 dark:border-slate-800/80">
        <Input
          ...
          className="h-10 rounded-lg border-slate-200/80 bg-white/90 text-[13px] shadow-none focus-visible:ring-0 dark:border-slate-800 dark:bg-slate-900/90"
          placeholder="搜索策略..."
        />
      </div>
      ...
      <div className="mt-auto border-t border-slate-200/80 bg-slate-50/80 p-4 dark:border-slate-800/80 dark:bg-slate-950/50">
        <div className="mb-3 flex items-center justify-between">
          ...
          >
            {editorState.status === "active" ? "ACTIVE" : editorState.status.toUpperCase()}
          </span>
        </div>
        <div className="space-y-2 text-[11px]">
          <div className="flex justify-between gap-2">
            <span className="text-slate-500 dark:text-slate-400">Version</span>
            <span className="font-medium text-slate-900 dark:text-slate-200">{editorState.version}</span>
          </div>
          <div className="flex justify-between gap-2">
            <span className="text-slate-500 dark:text-slate-400">Last Modified</span>
            <span className="max-w-[58%] text-right font-medium text-slate-900 dark:text-slate-200">
              {formatTime(selectedStrategy?.updated_at || null)}
            </span>
          </div>
          <div className="flex flex-wrap gap-1 pt-1">
            {tagChips.length ? (
              tagChips.slice(0, 6).map((t) => (
                <span
                  key={t}
                  className="rounded bg-slate-200 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-slate-600 dark:bg-slate-800 dark:text-slate-400"
                >
                  {t}
                </span>
              ))
  ...
  const mainPanel = (
    <div className="flex h-full min-w-0 flex-col overflow-hidden bg-white dark:bg-slate-950">
      <div className="shrink-0 border-b border-slate-200 bg-[#f6f7f9] px-5 py-3 dark:border-slate-800 dark:bg-slate-950">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-[18px] font-semibold tracking-tight text-slate-900 dark:text-slate-50">
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="space-y-4 p-5">
          ...
          >
            <TabsList
              variant="line"
              className="mb-4 h-auto w-full justify-start gap-6 rounded-none border-0 border-b border-slate-200 bg-transparent p-0 pb-0 dark:border-slate-800"
            >
              <TabsTrigger
                value="meta"
                className="rounded-none border-b-2 border-transparent px-0.5 pb-3 text-[13px] font-medium data-active:border-slate-900 data-active:bg-transparent dark:data-active:border-slate-100"
              >
                策略信息
              </TabsTrigger>
              <TabsTrigger
            ...
            <TabsContent value="code" className="m-0 outline-none">
              <div className="overflow-hidden rounded-xl border border-slate-200 bg-slate-950 dark:border-slate-800">
                <div className="flex items-center justify-between gap-4 border-b border-white/10 px-4 py-3">
                  <div className="text-[13px] font-medium text-slate-200">源码</div>
                  <div className="flex items-center gap-2 rounded-md border border-white/10 px-2.5 py-1 text-[11px] text-slate-400">
                    <Code2 size={14} />
                    Python
                  </div>
                </div>
