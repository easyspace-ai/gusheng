...
export function MajorIndexChart({
  ...
  useEffect(() => {
    ...
    chart.addSeries(LineSeries, { color: '#5272d6', lineWidth: 3, priceLineVisible: false, lastValueVisible: false }, 0)
    chart.addSeries(LineSeries, { color: '#8ac86c', lineWidth: 3, priceLineVisible: false, lastValueVisible: false }, 0)
    chart.addSeries(LineSeries, { color: '#f4c35b', lineWidth: 3, priceLineVisible: false, lastValueVisible: false }, 0)
    chart.addSeries(LineSeries, { color: '#f18a8a', lineWidth: 3, priceLineVisible: false, lastValueVisible: false }, 0)
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className={`text-[22px] font-semibold ${titleTone}`}>
          {name} {latest?.date || '--'} {formatNumber(latest?.close)} {latestChangePct == null ? '--' : formatPercent(latestChangePct)}
        </div>
        <div className="flex flex-wrap items-center gap-5 text-[17px] font-semibold text-slate-300">
          {legend.map((item) => (
            <div key={item.label} className="flex items-center gap-2">
              <span
                className={item.solid ? 'h-7 w-12 rounded-md' : 'h-4 w-4 rounded-full border-[4px] bg-white'}
                style={{
      ...
      <div className="relative overflow-hidden rounded-[30px] border border-slate-200 bg-white px-4 py-6">
        <div ref={containerRef} style={{ height: 720 }} />
        {loading ? (
          <div className="absolute inset-4 flex items-center justify-center gap-3 rounded-[24px] bg-white/88 text-sm text-slate-500 backdrop-blur-sm">
            <Loader2 className="h-4 w-4 animate-spin" />
            正在加载{name}K线...
          </div>
        ) : null}
        {!loading && error ? (
          <div className="absolute inset-4 flex items-center justify-center rounded-[24px] border border-rose-200 bg-rose-50 px-6 text-center text-base text-rose-600">
            {error}
          </div>
        ) : null}
        {!loading && !error && !points.length ? (
          <div className="absolute inset-4 flex items-center justify-center rounded-[24px] border border-dashed border-slate-200 bg-white/92 px-6 text-center text-base text-slate-500">
            暂无{name}K线数据
          </div>
        ) : null}
      </div>
      ...
      {latest ? (
        <div className="grid gap-3 md:grid-cols-4">
          <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3">
            <div className="text-xs uppercase tracking-[0.16em] text-slate-400">开盘 / 收盘</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">
              {formatNumber(latest.open)} / {formatNumber(latest.close)}
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3">
            <div className="text-xs uppercase tracking-[0.16em] text-slate-400">最高 / 最低</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">
              {formatNumber(latest.high)} / {formatNumber(latest.low)}
            </div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3">
            <div className="text-xs uppercase tracking-[0.16em] text-slate-400">成交量(万)</div>
            <div className="mt-2 text-lg font-semibold text-slate-900">{formatNumber((latest.volume || 0) / 10000, 2)}</div>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white/80 px-4 py-3">
            <div className="text-xs uppercase tracking-[0.16em] text-slate-400">涨跌</div>
            <div className={`mt-2 text-lg font-semibold ${titleTone}`}>
