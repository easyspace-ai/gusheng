...
_auth_disable_raw = os.getenv("TRADINGAPI_DISABLE_AUTH")
_auth_disabled_default = True
if _auth_disable_raw is not None:
    _auth_disabled_default = _auth_disable_raw.strip().lower() in {"1", "true", "yes", "on"}

_DEV_USER_ID = os.getenv("TRADINGAPI_DEV_USER_ID", "00000000-0000-0000-0000-000000000001")
_DEV_USER_EMAIL = os.getenv("TRADINGAPI_DEV_USER_EMAIL", "dev@local")
...
app = FastAPI(
    ...
    version=APP_VERSION,
    lifespan=lifespan,
    docs_url=None if not _auth_disabled_default else "/docs",
    redoc_url=None if not _auth_disabled_default else "/redoc",
    openapi_url=None if not _auth_disabled_default else "/openapi.json",
)


def _auth_disabled() -> bool:
    return _auth_disabled_default


def _get_or_create_dev_user(db: Session) -> UserDB:
    now = datetime.now(timezone.utc)
    user = db.query(UserDB).filter(UserDB.id == _DEV_USER_ID).first()
    if user:
        return user
...
def _dev_login_response(db: Session) -> AuthVerifyCodeResponse:
...
async def _ai_extract_symbol_and_date_streaming(
    ...
    """
    local_symbol, local_date, local_horizons, local_focus_areas, local_specific_questions, local_user_context = _resolve_stock_intent_local(text)
    if local_symbol:
        _log(f"[StockExtract streaming] local match: '{text[:40]}' -> {local_symbol}")
        return local_symbol, local_date, local_horizons, local_focus_areas, local_specific_questions, local_user_context
...
def _ai_extract_symbol_and_date(
    ...
    local_symbol, local_date, local_horizons, local_focus_areas, local_specific_questions, local_user_context = _resolve_stock_intent_local(text)
    if local_symbol:
        _log(f"[StockExtract] local match: '{text[:40]}' -> {local_symbol}")
        return local_symbol, local_date, local_horizons, local_focus_areas, local_specific_questions, local_user_context
...
def list_reports(
    ...
    Development mode keeps this endpoint public so the analysis history tab
    can work without a browser login round-trip. The returned records are
    still backed by the TradingAPI database.
    """
    total = report_service.count_reports(db=db, user_id=None, symbol=symbol)
...
def delete_report_endpoint(
    ...
    """删除报告."""
    success = report_service.delete_report(db, report_id, user_id=None)
    if not success:
        raise HTTPException(status_code=404, detail="报告不存在")
...
def request_login_code(request: AuthRequestCodeRequest, db: Session = Depends(get_db)):
    ...
    return {
        "message": "开发模式已自动登录",
        "access_token": auth_service.create_access_token(dev_user),
...
def run() -> None:
    ...
    port = int(os.getenv("TRADINGAPI_PORT") or os.getenv("PORT") or "8000")
