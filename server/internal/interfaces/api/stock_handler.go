...
func (h *StockHandler) GetKlineCompat(c *gin.Context) {
	...
	endTime, err := parseKlineDate(endDate)
	if err != nil {
	...
	startTime, err := parseKlineDate(startDate)
	if err != nil {
...
func (h *StockHandler) buildKlineCompatResponse(rawSymbol string, startTime time.Time, endTime time.Time, startDate string, endDate string) (klineCompatResponse, error) {
	symbol := normalizeKlineSymbol(rawSymbol)
	data, err := h.stockRepo.GetKLine(symbol, "day", 500, "qfq")
	if err != nil {
		return klineCompatResponse{}, err
	}

	candles := make([]klineCompatCandle, 0, len(data.List))
	...
	if len(candles) == 0 {
		return klineCompatResponse{}, fmt.Errorf("no kline data")
	}
...
func (h *StockHandler) Follow(c *gin.Context) {
	...
	if err := h.stockRepo.FollowStock(userId, code, name); err != nil {
		c.JSON(http.StatusOK, Error(err.Error()))
		return
	}
