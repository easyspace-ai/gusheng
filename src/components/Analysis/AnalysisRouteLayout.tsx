...
import TradingKlinePanel from '@/components/Analysis/TradingKlinePanel'
import {
...
export function AnalysisRouteLayout() {
  ...
  }, [historyLoadedRemote, historyLoading, rightView])
...
function AnalysisLeftPanel({
  ...
  return (
    <aside className="h-full overflow-y-auto border-r border-slate-200/80 bg-[#f7f9fc] dark:border-slate-800/80 dark:bg-slate-950/70">
      <div className="border-b border-slate-200/80 px-5 py-3.5 dark:border-slate-800/80">
        <div className="text-sm font-semibold tracking-[0.01em] text-slate-600 dark:text-slate-300">搜索</div>
      </div>

      <div className="border-b border-slate-200/80 px-5 py-5 dark:border-slate-800/80">
        <div className="flex items-center gap-3">
          <div className="flex min-w-0 flex-1 items-center gap-3 rounded-lg border border-slate-200 bg-white px-4 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.03)] dark:border-slate-800 dark:bg-slate-900">
            <Search className="h-4.5 w-4.5 shrink-0 text-slate-500" />
            <input
              value={manualSymbol}
              onChange={(e) => setManualSymbol(e.target.value.toUpperCase())}
              placeholder="输入股票代码..."
              className="w-full min-w-0 bg-transparent text-[14px] text-slate-800 outline-none placeholder:text-slate-400 dark:text-slate-100"
            />
          </div>
          <button
            onClick={handleAnalyze}
            className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-3 text-[14px] font-semibold text-white shadow-sm transition-all hover:opacity-95 dark:bg-white dark:text-slate-900"
          >
      ...
      <div className="p-5">
        <div>
          <div className="mb-4">
            <div className="text-[15px] font-semibold text-slate-900 dark:text-slate-50">自选列表</div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="inline-flex shrink-0 items-center gap-1 text-[12.5px] font-medium text-blue-600 dark:text-blue-300">分析 <ArrowRight className="h-3.5 w-3.5" /></span>
                  </div>
                </button>
              )
            })}
